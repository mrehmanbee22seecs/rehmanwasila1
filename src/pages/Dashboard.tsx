            ...
            {drafts.length > 0 && (
              <div className="luxury-card bg-white p-8">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-sm text-black/70 font-luxury-body">{drafts.length} draft{drafts.length !== 1 ? 's' : ''}</span>
                </div>

                <div className="space-y-4">
                  {drafts.map((draft) => (
                    <div key={draft.id} className="p-4 border-2 border-dashed border-gray-300 rounded-luxury hover:border-vibrant-orange transition-colors">
                      <div className="flex items-start justify-between mb-2">
                        <h4 className="font-luxury-heading text-black">{draft.title || 'Untitled Draft'}</h4>
                        <span className="px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                          Draft
                        </span>
                      </div>
                      <p className="text-sm text-black/70 font-luxury-body mb-2 line-clamp-2">{draft.description || 'No description'}</p>
                      <div className="flex items-center justify-between text-xs text-black/70">
                        <span className="font-luxury-body">
                          {draft.submissionType.charAt(0).toUpperCase() + draft.submissionType.slice(1)}
                        </span>
                        <span className="font-luxury-body">
                          Saved {draft.submittedAt?.toDate?.()?.toLocaleDateString() || 'recently'}
                        </span>
                      </div>
                    </div>
                  ))}
